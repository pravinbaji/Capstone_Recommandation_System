...
def prediction():
    ...
    if not (items is None):
        print(f"Fetching the Products please wait....{len(items)}")
        print(items)
        # data=[items.to_html(classes="table-striped table-hover", header="true",index=False)
        return render_template("index.html", Title="Following are the Top 5 Product Recomdations for User : " + request.form['userName']  , column_names=items.columns.values, row_data=list(items.values.tolist()),
                               zip=zip)
    else:
        if user == "":
            return render_template("index.html", message="Username cannot be blank")

        return render_template("index.html", message="Invalid User :: " + request.form['userName'] + " , Product cannot be Recomanded")
